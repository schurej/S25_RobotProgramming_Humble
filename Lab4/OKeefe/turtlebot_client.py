...
class TurtleClient(Node):
	...
	### MOKEEFE 2/28/25 Adding code to set Color & Pen Size Paramters ###
        self.declare_parameter('turtleColor', 'yellow', ParameterDescriptor(description= 'turtle color'))
        turtleColor = self.get_parameter('turtleColor').get_parameter_value().string_value
        self.turtle_display.color(turtleColor)
        #self.turtle.color(turtleColor)
  
        
        self.declare_parameter('penSize', 5, ParameterDescriptor(description= 'Size of Pen Trail'))
        penSize = self.get_parameter('penSize').get_parameter_value().integer_value
        self.turtle_display.pensize(penSize)
        
	### MOKEEFE 2/28/25 adding code to call SetColor service ###
