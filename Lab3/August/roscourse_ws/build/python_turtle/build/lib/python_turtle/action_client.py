...
class TurtleClient(Node):
    ...
    def goal_actioncall(self):
        ...
        for i in range(5):
            goal = Pose()
            goal.position.x = float(random.randint(math.floor(-1*self.screen.window_width()/2+20),math.floor(self.screen.window_width()/2-20)))
            goal.position.y = float(random.randint(math.floor(-1*self.screen.window_height()/2+20),math.floor(self.screen.window_height()/2-20)))
